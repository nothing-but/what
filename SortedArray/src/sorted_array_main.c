...
static  void free_array(SortedArray* array) {
  unsigned long  el_num =  sorted_array_size(array);
  for(unsigned long i=0;i<el_num;i++){
    struct record *array_element = (struct record *)sorted_array_get(array, i);
    free(array_element->string_field);
    free(array_element);
  }
  sorted_array_free_memory(array);
}

static  void print_array(SortedArray* array){
  unsigned long el_num =  sorted_array_size(array);
  ...
  for(unsigned long i=0;i<el_num;i++){
    array_element = (struct record *)sorted_array_get(array, i);
    printf("<%s,%d>\n",array_element->string_field,array_element->integer_field); 
  }
}

static void load_array(const char* file_name, SortedArray* array){
  char *read_line_p;
  char buffer[1024];
  int buf_size = 1024;
  FILE *fp;
  ...
  while(fgets(buffer,buf_size,fp) != NULL){
    ...
    sorted_array_add(array, (void*)record_p);
    free(read_line_p);
  }
  fclose(fp);
  printf("\nData loaded\n");
...
static  void test_with_comparison_function(const char* file_name, int (*compare)(const void*, const void*)) {
  SortedArray* array = sorted_array_create(compare);
